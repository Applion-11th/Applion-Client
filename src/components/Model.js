...
export default function Model({ scroll, ...props }) {
  ...
  useEffect(() => {
    if (hovered) group.current.getObjectByName(hovered).material.color.set("#A62121");
  ...
  useFrame((state) => {
    ...
    group.current.children[0].children.forEach((child, index) => {
      child.material.color.lerp(color.set(hovered === child.name ? "tomato" : "#942222"), hovered ? 0.1 : 0.1);
